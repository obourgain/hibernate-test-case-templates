...
public abstract class AbstractRowReader implements RowReader {
	...
	private EntityReferenceInitializer getInitializerByEntityReference(EntityReference targetEntityReference) {
		if ( entityInitializerByEntityReference == null ) {
			...
			// to block the first thread in the stop where you can trigger the race condition
			// as the field has been set, so the other thread won't enter this if block, but the Map is empty yet
			ORMUnitTestCase.LOCK.lock();

			// if reproducing with a debugger, remove the lock above and put a breakpoint on the line below. Same reason, field assigned but Map is empty yet
			for ( EntityReferenceInitializer entityReferenceInitializer : entityReferenceInitializers ) {
